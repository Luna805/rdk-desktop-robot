...
#define DISPLAY_LANDING_PEEK_US         (380LL * 1000LL)
#define DISPLAY_LANDING_HALF_REVEAL_US  (520LL * 1000LL)
#define DISPLAY_LANDING_FULL_REVEAL_US  (700LL * 1000LL)
#define DISPLAY_LANDING_DROP_US         (1500LL * 1000LL)
...
static int64_t s_landing_sequence_start_us;
static bool s_landing_sequence_done = true;
...
static int64_t display_service_landing_total_us(void);
...
static void display_service_update_landing_sequence_locked(int64_t now);
...
static int64_t display_service_landing_total_us(void)
{
    return DISPLAY_LANDING_PEEK_US +
           DISPLAY_LANDING_HALF_REVEAL_US +
           DISPLAY_LANDING_FULL_REVEAL_US +
           DISPLAY_LANDING_DROP_US;
}
...
static bool display_service_is_landing_sequence_active_locked(int64_t now)
{
    if (s_landing_sequence_done || (s_landing_sequence_start_us <= 0)) {
        return false;
    }

    if ((now - s_landing_sequence_start_us) >= display_service_landing_total_us()) {
        s_landing_sequence_done = true;
...
static void display_service_update_landing_sequence_locked(int64_t now)
{
    ...
    if (!display_service_is_landing_sequence_active_locked(now)) {
        display_service_hide_momo_locked();
        return;
    }

    int64_t phase_elapsed = now - s_landing_sequence_start_us;
    lv_coord_t momo_y = DISPLAY_MOMO_HIDDEN_Y;
    display_momo_face_t momo_face = DISPLAY_MOMO_FACE_SLEEP;
    int64_t reveal_total_us = DISPLAY_LANDING_PEEK_US +
                              DISPLAY_LANDING_HALF_REVEAL_US +
                              DISPLAY_LANDING_FULL_REVEAL_US;

    if (phase_elapsed < reveal_total_us) {
        ...
        if (phase_elapsed < DISPLAY_LANDING_PEEK_US) {
            momo_face = DISPLAY_MOMO_FACE_SLEEP;
        } else if (phase_elapsed < (DISPLAY_LANDING_PEEK_US + DISPLAY_LANDING_HALF_REVEAL_US)) {
            momo_face = DISPLAY_MOMO_FACE_CURIOUS;
        } else {
            int64_t full_reveal_elapsed = phase_elapsed - DISPLAY_LANDING_PEEK_US - DISPLAY_LANDING_HALF_REVEAL_US;
            momo_face = (full_reveal_elapsed < (DISPLAY_LANDING_FULL_REVEAL_US / 2))
                            ? DISPLAY_MOMO_FACE_CURIOUS
                            : DISPLAY_MOMO_FACE_HAPPY;
        }
    } else {
        phase_elapsed -= reveal_total_us;
        int32_t progress = display_service_ease_in_out_1000((int32_t)((phase_elapsed * 1000LL) / DISPLAY_LANDING_DROP_US));
...
{
    if (state == ROBOT_STATE_LANDING) {
        if (s_last_status_state != ROBOT_STATE_LANDING) {
            s_landing_sequence_start_us = now;
            s_landing_sequence_done = false;
            s_last_interaction_us = now;
            display_service_clear_touch_face_locked();
            display_service_clear_auto_face_locked();
            display_service_stop_blink_locked();
        }
    } else if (!s_landing_sequence_done || (s_momo_root != NULL)) {
        s_landing_sequence_done = true;
        display_service_hide_momo_locked();
    }

    display_service_update_landing_sequence_locked(now);
    if (display_service_is_landing_sequence_active_locked(now)) {
        display_service_apply_face_locked(DISPLAY_FACE_SLEEP, now);
        if (state != s_last_status_state) {
