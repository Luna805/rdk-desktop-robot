...
#define ROBOT_LINK_UART_PORT        UART_NUM_0
#define ROBOT_LINK_UART_RX_BUF_SIZE 256
...
static bool s_uart_link_ready = false;
...
static robot_state_t robot_link_state_from_token(const char *token, bool *matched)
{
    ...
    if ((strcasecmp(token, "wake") == 0) || (strcasecmp(token, "awake") == 0) ||
        (strcasecmp(token, "landing") == 0) || (strcasecmp(token, "land") == 0) ||
        (strcasecmp(token, "trigger") == 0)) {
        return ROBOT_STATE_LANDING;
    }
...
void robot_link_init(void)
{
    ...
    esp_err_t err = uart_param_config(ROBOT_LINK_UART_PORT, &uart_cfg);
    if (err == ESP_OK) {
        err = uart_set_pin(ROBOT_LINK_UART_PORT,
                           UART_PIN_NO_CHANGE,
                           UART_PIN_NO_CHANGE,
                           UART_PIN_NO_CHANGE,
                           UART_PIN_NO_CHANGE);
    ...
    if (err == ESP_OK) {
        err = uart_driver_install(ROBOT_LINK_UART_PORT,
                                  ROBOT_LINK_UART_RX_BUF_SIZE,
                                  0,
                                  0,
                                  NULL,
                                  0);
    ...
    if ((err == ESP_OK) || (err == ESP_ERR_INVALID_STATE)) {
        s_uart_link_ready = true;
        ESP_LOGI(TAG, "Robot link ready on UART0");
    } else {
        ESP_LOGW(TAG, "UART0 init failed, continue with USB Serial JTAG only: %s", esp_err_to_name(err));
    }

    ESP_LOGI(TAG, "Send one line state via UART0 or USB Serial JTAG: idle/listening/landing/wake/processing/speaking/happy/sleep/error");
}
...
robot_state_t robot_link_poll(void)
{
    ...
    if (!s_usb_link_ready && !s_uart_link_ready) {
        return s_current_state;
    }
    ...
    if (s_uart_link_ready) {
        int read_len = uart_read_bytes(ROBOT_LINK_UART_PORT, (uint8_t *)rx_buf, sizeof(rx_buf), 0);
        if (read_len > 0) {
            robot_link_consume_rx(rx_buf, (size_t)read_len);
        }
    }
